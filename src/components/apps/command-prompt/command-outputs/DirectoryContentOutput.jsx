...
const DirectoryContentOutput = ({ currentPath, command, path, args, getFolder }) => {
  ...
  useEffect(async () => {
    if (command === 'dir') {
      ...
      try {
        ...
        setCommandOutput([
          {
            folderName: Path.join(currentPath, path ? path.replaceAll('"', '') : ''),
      ...
      {content &&
        content.map((fso) => (
          <div key={fso.node} className='fso-info'>
            <span className='modified-date'>{moment(fso.mtime).format('MM/DD/yy')}</span>
            <span className='modified-time'>{moment(fso.mtime).format('hh:mm A')}</span>
            <span
              className='file-type'
              style={{
            ...
            >
              {fso.type === 'DIRECTORY' ? '<' + fso.type.substring(0, 3) + '>' : fso.size}
            </span>
            <span>
              {command === 'ls' && fso.name.includes(' ') ? `"${fso.name}"` : fso.name}
            </span>
          </div>
        ))}
