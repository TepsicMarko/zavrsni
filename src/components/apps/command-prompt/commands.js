...
const commands = (
  ...
  chdir: {
    ...
    fn: (path, ...args) => (!path ? currentPath : changePath([path, ...args].join(' '))),
  },
  ...
  ls: {
    ...
    fn: (path, ...args) =>
      !path ? listFolderContents('ls') : listFolderContents('ls', path, ...args),
  },
  dir: {
  ...
  mkdir: {
    ...
    fn: (...args) =>
      args.length ? createNewFolders('mkdir', ...args) : 'mkdir: missing operand',
  },
  md: {
  ...
  tasklist: {
    ...
    fn: () => (
      <div style={{ maxWidth: 'fit-content' }}>
        ...
        {Object.entries(processes).map(([processName, processInstances]) =>
          Object.entries(processInstances).map(([processInstanceId, processInstance]) =>
            processInstance.isChildProcess ? null : (
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <div>{processName}</div>
                <div>{processInstanceId}</div>
              </div>
            )
          )
        )}
  ...
  taskkill: {
    ...
    fn: (flag, ...process) =>
      flag
        ? ['/im', '/pid'].includes(flag.toLowerCase())
          ? process
            ? killTasks(flag.toLowerCase(), ...process)
            : `ERROR: Invalid syntax. Value expected for  ${flag}.`
          : 'ERROR: Invalid argument/option - ' + flag
        : 'ERROR: Invalid syntax. Neither /IM nor /PID were specified.',
  },
